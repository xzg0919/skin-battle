package com.tzj.point.controller;
 ...
 **/
@Controller
@RequestMapping(value = "/jfapp/api")
public class JfappApiController extends ApiController {
    ...
    protected void initApiConfig(ApiConfig apiConfig) {
        ...
        appSecretStore.put("app_id_1", "sign_key_jfapp");
        apiConfig.addAppSecret(appSecretStore);

        ApiKeys apiKeys=new ApiKeys();
        apiKeys.setTokenSignKey(false); //使用 appid的签名key
        apiKeys.setProduceSignKey(JFAPP_API_TOKEN_SIGN_KEY);
        apiKeys.setTokenCyptoKey(JFAPP_API_TOKEN_CYPTO_KEY);
        apiKeys.setTokenSecretKey(JFAPP_API_TOKEN_SECRET_KEY);
