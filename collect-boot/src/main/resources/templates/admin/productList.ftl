...
<body>
    <section id="warp">
        ...
        <header class="head">
                ...
                <ul class="nav nav-pills">
                    <li role="presentation" class="activea"><a href="#">待发货订单</a></li>
                    <li role="presentation"><a href="#" onclick = "complete()">已完成订单</a></li>
                </ul>
                   ...
                   <#list ProductOrderList as order>
                    ...
                    <td>
                        <div  class="btn-group  lllll">
                            <button type="button" class="btn  names btn-default" id = "${order.id}" >${companyList[0]}</button>
                            <button type="button" class="btn btn-default dropdown-toggle" 
                                data-toggle="dropdown">
                                <span class="caret"></span>
                            </button>
                            <ul class="dropdown-menu" role="menu">
                            	<#list companyList as a>
                             	   <li><a href="#" onclick="company('${a}','${order.id}')" >${a}</a></li>
                                </#list>
                            </ul>
                        </div>
                    </td>
                    <td class="xq">
                           单号： <input type="text" class="form-control  deliverInput" placeholder="" id="${order.id}1" onkeypress="getKey('${order.id}');"><span id="deliver" class="greenC" onclick="sendProduct('${order.id}')">发货</span>
                        <!-- <span id="info" class="greenC">详情</span> -->
                    </td>
                    </tr>
                    </#list>
    ...
    <#if count != 0 >
   		 <input type="hidden" id="count" value="${ProductOrderList[0].id}"/>
	</#if>
</body>
...
//下拉选择时赋值
function company(companyName,id){
	document.getElementById(id).value = companyName
	document.getElementById(id).innerHTML = companyName
};
//条件模糊查询事件
function search(){
	var mobile = document.getElementById("mobile").value
	var productName = document.getElementById("productName").value
	window.location = "/product/order/getProductOrderList?mobile="+mobile+"&productName="+productName
}
//点击发货事件
function sendProduct(id){
	var companyName = document.getElementById(id).innerHTML
	var orderNum = document.getElementById(id+"1").value
	if(companyName == "" || companyName == undefined || companyName == null || orderNum == "" || orderNum == undefined || orderNum == null){
		return alert("快递单号不能为空")
	}
	window.location = "/product/order/updateProductOrderById?orderNum="+orderNum+"&orderCompany="+companyName+"&id="+id
}
//订单号回车提交发货事件
function getKey(id)
{
...
//跳转到已发货列表
function complete(){
	window.location = "/product/order/sendProductOrderList"
}
</script>
