 ...
 **/
@Controller
@RequestMapping(value = "/picc/api")
public class PiccCompanyApiController extends ApiController {

    @Resource(name="apiNonceServiceImpl")
    private NoceService noceService;

    @Resource(name="piccSubectServiceImpl")
    ...
    protected void initApiConfig(ApiConfig apiConfig) {
        ...
        appSecretStore.put("app_id_7", "sign_key_77bbccdd");
        apiConfig.addAppSecret(appSecretStore);

        ApiKeys apiKeys=new ApiKeys();
        apiKeys.setTokenSignKey(false); //使用 appid的签名key
        apiKeys.setProduceSignKey(PICC_API_TOKEN_SIGN_KEY);
        apiKeys.setTokenCyptoKey(PICC_API_TOKEN_CYPTO_KEY);
        apiKeys.setTokenSecretKey(PICC_API_TOKEN_SECRET_KEY);
