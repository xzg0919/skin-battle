...
public class TerminalApiController extends ApiController {

    @Resource(name="apiNonceServiceImpl")
    private NoceService noceService;

    @Resource(name="terminalApiSubjectServiceImpl")
    ...
    protected void initApiConfig(ApiConfig apiConfig) {
        ...
        appSecretStore.put("app_id_6", "sign_key_88bbccdd");
        apiConfig.addAppSecret(appSecretStore);

        ApiKeys apiKeys=new ApiKeys();
        apiKeys.setTokenSignKey(false); //使用 appid的签名key
        apiKeys.setProduceSignKey(TERMINAL_API_TOKEN_SIGN_KEY);
        apiKeys.setTokenCyptoKey(TERMINAL_API_TOKEN_CYPTO_KEY);
        apiKeys.setTokenSecretKey(TERMINAL_API_TOKEN_SECRET_KEY);
