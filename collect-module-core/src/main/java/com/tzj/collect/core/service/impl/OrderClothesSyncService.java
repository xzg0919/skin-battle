...
public class OrderClothesSyncService implements OrderSyncService {
    ...
    public String OrderSync(Order order, String status) {
        ...
        orderSyncBizContent.setOrder_detail_url("alipays://platformapi/startapp?appId=2018060660292753&page=pages/view/orderDetails/rubbish/rubbish%3Fid%3D" + orderId);
        ...
        serviceProductInfo.setGoods_desc("废旧衣物回收");
        serviceProductInfo.setGoods_name(category == null ? "废旧衣物" : category.getName());
        Integer qty = 1;
        if (order.getQty() != null) {
            qty = order.getQty();
        }
        if (order.getExpressAmount() != null && order.getExpressAmount().intValue() > 0) {
            qty = order.getExpressAmount().intValue();
        }
        serviceProductInfo.setQuantity(qty + "");
        ...
        log.info("废旧衣物订单回流请求参数：" + com.alibaba.fastjson.JSONObject.toJSONString(orderSyncBizContent));
        ...
        if ((!response.isSuccess() || !"10000".equals(response.getCode())) && "CREATE".equals(status)) {
            throw new ApiException("操作失败，订单无法回流！");
        }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(DateUtils.formatDate(calendar.getTime(), "yyyy-MM-dd HH:mm:ss"));
    }
}
